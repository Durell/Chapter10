/* File Name: FileCatch.java
   Programmer:June Porto

   Problem Statement:
   Demonstrate how you must catch or declare checked exceptions which
     include reading in from a file. The compiler will make sure if the file
     can't be found (or doesn't exist), that this exception will be caught or
     declared with a throws clause.
   In the code below, the try and the catch block are removed from the code. 
   This causes the compiler to issue an unreported exception error, namely a
